...
Document these explicitly — interviewers will ask "why did/didn't you use X?"

```
Edge Runtime:         NOT used for any contact routes.
                      Reason: Neon requires TCP connections. Edge Runtime has no
                      native TCP — requires Neon's HTTP adapter, which adds a
                      round-trip and can't use the connection pool. Not worth it.
                      Use standard Node.js serverless functions.

Partial Prerendering: NOT used on /contacts, /segments, or /outreach.
                      Reason: entire page content is user-specific and dynamic.
                      PPR shines with a static shell + dynamic islands. There is
                      no static shell worth prerendering on these pages.
                      COULD apply to /dashboard: the nav and layout are static,
                      the stat cards are dynamic. Good PPR candidate for a demo.

Vercel KV/Redis:      NOT used.
                      Reason: no caching layer needed at 200k rows. Neon with
                      proper indexes (ivfflat, GIN trigram, partial pending index)
                      handles all query patterns. Adding Redis adds cache
                      invalidation complexity with no measurable gain at this scale.
                      Would revisit at 1M+ contacts if query latency degrades.

Vercel Cron:          USED for incremental dedup and embedding retry.
                      Schedule: embed at 2am UTC, dedup at 3am UTC (staggered
                      so they don't compete for DB connections).
                      Why cron not webhooks: jobs are triggered by time, not events.
                      A new import queues work; cron does the work.
                      Free on Pro plan.

Vercel Blob:          USED for original CSV storage.
                      Why: cheap, durable, no DB overhead. Provides an audit trail
                      and the ability to re-import if processing fails.
                      Private access only — URLs are gated by auth.

Function maxDuration: SET to 300s for dedup and embed routes (Vercel Pro max).
                      Each invocation processes one batch (BATCH_SIZE contacts)
                      and checkpoints. The cron retriggers nightly to continue.
                      Import capped at 120s — it returns after queuing, not processing.

AI SDK Gateway:       Optional. Useful for demonstrating multi-model fallback
                      (claude-sonnet-4-6 primary → gpt-4o-mini fallback on timeout).
...
Prepare these for the interview. Have answers for both audiences.

### VP of Engineering
- Next.js App Router with Suspense streaming: dashboard stats load progressively, first paint is instant regardless of query time
- Neon pgvector: single DB handles relational queries AND semantic similarity search — no separate vector store to operate or keep in sync
- AI SDK `streamText`: perceived performance on AI features is excellent, first tokens in ~300ms vs 3-5s spinner with `generateText`
- Incremental dedup job: at 200k contacts, nightly dedup checks only new arrivals (O(new_imports)), not the full corpus (O(total)). Scales indefinitely.
- Chunked + checkpointed jobs: Vercel function timeouts are not an obstacle. Each invocation makes progress; cron continues the work. The system recovers from any failure.
- Partial index on `embedding_status = 'pending'`: embedding retry cron stays fast at any table size — it never scans the rows it already processed.
- Eval harness in `__tests__/evals.test.ts`: NL search safety and hallucination regressions are caught in CI before they reach production

### CFO
- Replaces: manual spreadsheet merging (hours), blind broadcast emails (low conversion), guessing which segment to invite (wasted effort)
- Time saved: segment definition + draft outreach goes from ~2 hours manually to ~2 minutes
- Infrastructure cost: ~$20/month Vercel Pro + Neon Free tier + ~$0.02 per outreach draft (claude-sonnet-4-6 pricing)
- Embedding cost: one-time ~$4 for 200k contacts (text-embedding-3-small at $0.02/1M tokens), then ~$0.02 per incremental import batch
- Multi-tenant ready: row-level user isolation (Clerk user_id on every query) means this architecture supports multiple teams with zero cross-contamination
- Enterprise path: row-level isolation + private Blob storage + no PII sent to AI models satisfies most enterprise data handling requirements out of the box

### Enterprise / Security-conscious buyer
- Contact data never leaves your private Neon database
- AI models only see name, role, company, and events attended — never email addresses
- Original CSVs stored in private Vercel Blob with auth-gated access (audit trail for data provenance questions)
- Each user sees only their own data — the `user_id` filter is enforced at the query layer, not the application layer
...
3. **CSV upload + schema mapper** (`/import` page + `/api/import`) — this is the "wow" demo moment
4. **Contact table with basic search** (`/contacts` page with keyset pagination)
5. **NL search evals** (`__tests__/evals.test.ts` NL cases — write these first, then make them pass)
6. **NL search** (`lib/nl-search.ts` + guardrails)
7. **Embedding pipeline** (`lib/embeddings.ts` + batch job with `unnest()` bulk update)
8. **Dedup job** (`lib/dedup.ts` incremental + chunked + candidate review UI)
9. **Segment builder** (`/segments` page)
10. **Outreach drafter** (`/outreach` page with streaming)
11. **Dashboard** (SSR + Suspense — do this last, easiest to build)
12. **Hallucination + dedup evals** (complete the eval suite, run `npm test`)
13. **`vercel.json` + cron routes** (`/api/cron/embed`, `/api/cron/dedup`)
